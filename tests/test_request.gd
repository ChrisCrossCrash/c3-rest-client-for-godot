...
class TestRequest extends GutTest:
	...
	func test_returns_rest_response() -> void:
		var result := await client.request("/embeddings", "POST")
		assert_is(result, C3RestClient.RestResponse)

	func test_uses_the_given_path() -> void:
		client.base_url = "http://example.com/v1"
		await client.request("/embeddings", "POST")
		assert_eq(
	...
	func test_method_is_case_insensitive() -> void:
		await client.request("/files/abc", "delete")
	...
	func test_empty_api_key_omits_auth_header() -> void:
		client.api_key = ""
		await client.request("/embeddings", "POST")
		var headers: PackedStringArray = client.request_log[0]["headers"]
		for header in headers:
			assert_false(header.begins_with("Authorization:"))
	...
	func test_empty_body_response_is_success() -> void:
		...
		var result := await client.request("/files/abc", "DELETE")
		assert_true(result.ok)
	...
	func test_invalid_method_fails_without_sending() -> void:
		var result := await client.request("/embeddings", "FETCH")
...
## Tests for [C3RestClient.RestResponse] defaults.
class TestRestResponse extends GutTest:
	func test_default_ok() -> void:
		assert_true(C3RestClient.RestResponse.new().ok)

	func test_default_error() -> void:
		assert_null(C3RestClient.RestResponse.new().error)

	func test_default_raw_body() -> void:
		assert_eq(C3RestClient.RestResponse.new().raw_body, {})
